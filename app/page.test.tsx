...
describe("Home", () => {
  ...
  it("turns a stuck question into an actionable constraint map", async () => {
    ...
    expect(screen.getByLabelText("图谱控制")).toBeInTheDocument();
    ...
    expect(screen.getByRole("slider", { name: "图谱缩放" })).toHaveValue("140");
    fireEvent.click(screen.getByRole("button", { name: "自动旋转图谱" }));
    expect(screen.getByRole("button", { name: "自动旋转图谱" })).toHaveAttribute("aria-pressed", "false");
    fireEvent.click(screen.getByRole("button", { name: "复位图谱视角" }));
    expect(screen.getByRole("slider", { name: "字体大小" })).toHaveValue("130");
    expect(screen.getByRole("slider", { name: "图谱缩放" })).toHaveValue("100");
