...
export default function DisableTower3D({ input }: { input: Analysis }) {
  ...
  useEffect(() => {
    ...
    for (const layer of model.layers) {
      ...
      const ceilingGrid = new THREE.GridHelper(6.9, 7, layer.color, layer.color);
      ceilingGrid.position.y = layer.y + .94 + ceilingThickness / 2;
      const ceilingMaterial = ceilingGrid.material as THREE.Material;
      ceilingMaterial.transparent = true;
      ceilingMaterial.opacity = .08 + layer.ceilingStrength * .14;
    ...
    const onClick = (event: MouseEvent) => {
      ...
      const layerID = raycaster.intersectObjects(clickable, false)[0]?.object.userData.layerID as number | undefined;
  ...
  return <section className="disable-tower" aria-labelledby="tower-title">
    <canvas ref={canvasRef} aria-label="3D Disable 归因塔" />
    <header className="tower-heading"><div><Layers3 size={17} /><div><p>主视图</p><h3 id="tower-title">3D Disable 塔</h3></div></div><button aria-label="重置3D视角" title="重置视角" onClick={resetView}><Box size={16} /></button></header>
    <div className="tower-target"><Crosshair size={14} /><span>当前问题</span><strong>{model.target}</strong></div>
    <nav className="tower-layers" aria-label="3D层级选择">{model.layers.map((layer, index) => <button className={selected?.id === layer.id ? "is-active" : ""} onClick={() => setSelectedID(layer.id)} key={layer.id}><span>{String(index + 1).padStart(2, "0")}</span><strong>{layer.label}</strong><b>{layer.share}%</b></button>)}</nav>
    {selected && <aside className="tower-detail"><header><i style={{ background: `#${selected.color.toString(16).padStart(6, "0")}` }} /><span>第 {model.layers.findIndex((layer) => layer.id === selected.id) + 1} 层</span><strong>{selected.share}% 归因</strong></header><h4>{selected.label}</h4><p>{selected.description}</p><div><span>天花板强度 {Math.round(selected.ceilingStrength * 100)}</span>{selected.nodes.slice(0, 3).map((node) => <span key={node.id}>{node.label} · {interventionLabels[node.intervention]}</span>)}</div></aside>}
